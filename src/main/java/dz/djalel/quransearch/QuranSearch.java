...
public class QuranSearch {
    ...
    /// private final xxx settings;                     /* TODO: pass a ref to user settings from context */

    public QuranSearch(final XComponentContext context) throws IOException
    {
...
//        long count = 0;
       /*
        ...
        File quranFile = FileTools.getFilePath("data/quran/quran.txt", this.context);
        InputStream is = new FileInputStream(quranFile);
...
//                "Quran text length = " + quran.length());
    ...
    public String getAyaPrefix(int surah, int aya)
    {
        return String.format(Locale.getDefault(),
            "%s [%d]: ",
            SURAH_NAME[surah - 1][rasm == Rasm.IMLA ? 0 : 1],
            aya);
    }
    ...
    private void readRasmFile(List<AyaMatch> results)
    {
        ...
        try {
            String filePath = rasm == Rasm.UTHMANI ? "data/quran/uthmani.zip" : "data/quran/simple.zip";
            File quranFile = FileTools.getFilePath(filePath, this.context);
            InputStream is = new FileInputStream(quranFile);
            ZipInputStream zi = new ZipInputStream(is);
            InputStreamReader in = new InputStreamReader(zi, StandardCharsets.UTF_8);
